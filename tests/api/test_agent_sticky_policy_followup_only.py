...
def test_agent_sticky_policy_followup_only_doc_type_inherits(
    ...
    first_resp = client.post(
        "/api/agent/run",
        json={
            "message": "SUPRA N manual equip id EPAG50 점검",
            "auto_parse": True,
            "max_attempts": 0,
            "top_k": 2,
    ...
    followup_resp = client.post(
        "/api/agent/run",
        json={
            "message": "Tell me more about that document",
            "auto_parse": True,
            "max_attempts": 0,
            "top_k": 2,
            "thread_id": thread_id,
            "chat_history": [
                {
                    "user_text": "SUPRA N manual equip id EPAG50 점검",
                    "assistant_text": "점검 절차를 안내합니다.",
                    "doc_ids": ["doc-a"],
                }
            ],
            "use_canonical_retrieval": True,
    ...
    new_session_body = cast(dict[str, object], new_session_resp.json())
    assert cast(list[str], new_session_body["selected_devices"]) == []
    assert cast(list[str], new_session_body["selected_doc_types"]) == []
    assert cast(list[str], new_session_body["selected_equip_ids"]) == []
