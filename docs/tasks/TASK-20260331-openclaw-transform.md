...
- [ ] `retrieval_review` interrupt/`response_mode` parity 확보
- [ ] C-API-003 테스트 통과
...
- [ ] `/run` vs `/run/stream` final payload parity 확인
- [ ] `event_sink` 기반 observability parity 및 concurrency 테스트 통과
...
- API contract/regression suite: 미실행 (구현 단계 진입 후 Phase별 실행 예정)
