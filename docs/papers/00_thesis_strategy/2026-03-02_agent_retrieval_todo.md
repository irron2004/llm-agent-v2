...
- 범위:
  ...
  - stage2 / 문서내 재검색: 구현되지 않음 (N/A) - repo 검색 결과 0건
- 완료 기준:
  - `RRF fallback` 로그 0건
...
- 개선 방법:
  ...
  - stage2 미구현으로 인해 해당 스텝은 N/A (app-level RRF는 dense/sparse만 대상)
...
- 작업:
  ...
  3. baseline env var 조합 명시 (`bilingual/page_penalty/stage2/mq_mode`) - stage2는 미구현으로 N/A
...
- T2-3: stage2 병합 통일 - N/A (stage2 미구현)
- T2-4: 로그/trace 필드 확장
...
- Step 3 (stage2에도 RRF 적용): bilingual 실험에서 page-hit@1 하락의 직접 원인이 stage2 내 EN/KO score 병합이었으므로 핵심 스텝.
  - stage2 미구현으로 해당 스텝은 N/A. 현재 app-level RRF는 dense+sparse만 병합.
