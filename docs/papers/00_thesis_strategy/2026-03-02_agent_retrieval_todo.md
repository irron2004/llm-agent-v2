...
- 범위:
  ...
  - stage2 / 문서내 재검색: **구현 완료** (env flag `AGENT_SECOND_STAGE_DOC_RETRIEVE_ENABLED`, 기본 off)
- 완료 기준:
  - `RRF fallback` 로그 0건
...
- 개선 방법:
  ...
  - **구현 완료**: stage2 결과는 `rrf.merge_retrieval_results()`로 stage1과 동일한 rank-based RRF로 병합됨.
...
### 구현-note: App-level RRF

ES native RRF는 실행 경로에서 제거됨. 하이브리드 검색은 앱 레벨에서 dense+sparse 순위를 RRF로 fusion함.

- 구현 위치: `backend/llm_infrastructure/retrieval/engines/es_search.py` + `backend/llm_infrastructure/retrieval/rrf.py`
- 디버그 메타: `metadata.retrieval_meta.rrf.*` (rrf_dense_rank, rrf_sparse_rank, rrf_score, rrf_k)
...
### 구현-note: SOP intent policy

- Default mode: `soft`
- Modes: `off|soft|hard`
  - `off`: SOP intent inference 비활성화
  - `soft`: SOP 문서에 soft boost만 적용 (점수 상향)
  - `hard`: SOP 문서만 필터링 (hard filter)
- 설정 키: `AGENT_SOP_INTENT_MODE`, `AGENT_SOP_SOFT_BOOST_FACTOR`
- TS exclusion: TS(troubleshooting) 패턴 감지 시 SOP intent rules 적용 안 함
...
### 구현-note: Sticky policy

- Follow-up-only inheritance: `needs_history=true`일 때만 이전 세션의 필터 승계 허용
- New session: 이전 session의 selected_doc_types 미승계, 항상 빈값으로 시작
- Ordering: history_check가 auto_parse보다 먼저 실행되어 follow-up 판별 후 필터 승계 결정
...
### 구현-note: Evaluation harness

- 실행: `python scripts/evaluation/evaluate_sop_agent_page_hit.py --out-dir /tmp/agent_eval --use-testclient`
- JSONL 스키마 검증: `scripts/evaluation/validate_agent_eval_jsonl.py`
- 출력 파일: `agent_eval.jsonl`, `report.md`
- `--use-testclient`는 스키마/하네스 동작 검증용(의존성 스텁)이며, 실품질(doc/page hit) 평가는 실제 서버 HTTP 모드로 수행
...
- 작업:
  ...
  3. baseline env var 조합 명시 (`bilingual/page_penalty/stage2/mq_mode`) - stage2 구현 완료, 기본 off
...
- 구현 범위:
  ...
  3. stage2 병합 정책 통일 (동일 RRF 재사용) - **구현 완료** (env flag `AGENT_SECOND_STAGE_DOC_RETRIEVE_ENABLED`, 기본 off)
...
- T2-3: stage2 병합 통일 - **구현 완료** (rank-based RRF merge, `backend/llm_infrastructure/retrieval/rrf.py`)
- T2-4: 로그/trace 필드 확장
...
- Step 3 (stage2에도 RRF 적용): bilingual 실험에서 page-hit@1 하락의 직접 원인이 stage2 내 EN/KO score 병합이었으므로 핵심 스텝.
  - **구현 완료**: stage2는 `retrieve_node` 내에서 doc-local 재검색 후 rank-based RRF로 stage1과 병합. early-page penalty도 적용됨.
...
## Stage2 Retrieval + Early-Page Penalty (구현 완료, 2026-03-04)

### 개요
SOP 문서의 TOC/표지 페이지가 상위에 랭크되는 문제를 해결하기 위해, 2단계 검색(doc-local re-search) + early-page penalty를 구현했다. 기본 비활성화 상태이며 env flag로 제어한다.

### Env Vars (AgentSettings)

| 변수 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `AGENT_SECOND_STAGE_DOC_RETRIEVE_ENABLED` | bool | `false` | Stage2 doc-local 재검색 활성화 |
| `AGENT_EARLY_PAGE_PENALTY_ENABLED` | bool | `false` | 초기 페이지 penalty 활성화 |
| `AGENT_EARLY_PAGE_PENALTY_MAX_PAGE` | int | `2` | penalty 적용 대상 최대 페이지 번호 |
| `AGENT_EARLY_PAGE_PENALTY_FACTOR` | float | `0.3` | penalty 적용 시 score 승수 |
| `AGENT_SECOND_STAGE_MAX_DOC_IDS` | int | `1` | Stage2에서 재검색할 최대 doc 수 |
| `AGENT_SECOND_STAGE_TOP_K` | int | `50` | Stage2 doc-local 검색 top_k |

### 동작 흐름
1. **Stage1**: 기존 검색 (`search_queries` 기반)
2. **Early-page penalty** (활성 시): SOP 문서의 page ≤ `max_page`인 결과에 `factor` 곱하고 재정렬
3. **Stage2** (활성 시): Stage1 상위 doc_id에 대해 `doc_ids` 필터로 doc-local 재검색
4. **RRF 병합**: Stage1 + Stage2를 `rrf.merge_retrieval_results()`로 rank-based RRF merge
5. 기존 rerank + 최종 선택 로직 계속

### 구현 위치
- Settings: `backend/config/settings.py` (AgentSettings)
- Doc-ID filter: `backend/llm_infrastructure/retrieval/engines/es_search.py:build_filter`
- RRF merge: `backend/llm_infrastructure/retrieval/rrf.py:merge_retrieval_results`
- Stage2 로직: `backend/llm_infrastructure/llm/langgraph_agent.py:retrieve_node`

### SOP Soft Boost
- `sop_soft_boost_factor`: `1.05` → `1.30`으로 업데이트 (Pydantic Field 검증: gt=0.0, le=5.0)
- SOP intent 감지 시 해당 factor로 SOP 문서 score를 승수 적용

### Sticky Inheritance Scope (확정 정책)
- Follow-up (`needs_history=True`): `doc_type`만 상속
- New session / non-follow-up: 아무것도 상속하지 않음
- `devices`, `equip_ids`는 현재 요청에서 감지된 값만 사용 (이전 턴에서 상속하지 않음)

## MQ Mode Sweep Evaluator (구현 완료, 2026-03-04)

### 사용법
```bash
# MQ 모드별 sweep 실행 (off, fallback, on)
python scripts/evaluation/evaluate_sop_agent_page_hit.py \
    --out-dir /tmp/mq_sweep \
    --mq-modes off,fallback,on \
    --use-testclient  # 또는 --api-base-url http://... 로 HTTP mode
...
│   ├── agent_eval.jsonl
│   └── report.md
└── mq_ablation_report.md    ← 모드 간 비교 요약
```

### mq_ablation_report.md 해석
- 모드별 doc-hit@10, page-hit@1/@3/@5, Jaccard@k 비교
- TestClient 모드는 schema/tooling 검증용, 품질 판정은 반드시 HTTP mode로 수행

### JSONL 스키마 (정규화 완료)
- `answer`: 에이전트 응답 텍스트 (필수)
- `trace.retry_count`: 재시도 횟수 (필수, `metadata.attempts`에서 통합)
- 검증: `python scripts/evaluation/validate_agent_eval_jsonl.py --jsonl <path>`
...
- 2026-03-04 Stage2 retrieval + early-page penalty 구현 완료, env vars/동작 흐름/구현 위치 문서화.
- 2026-03-04 MQ sweep evaluator 구현 완료, `--mq-modes` 사용법/결과 구조/ablation report 해석 문서화.
- 2026-03-04 SOP soft boost 1.05→1.30 업데이트, sticky 상속 정책 확정, JSONL 스키마 정규화.
