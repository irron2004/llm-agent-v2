...
현재 구조에서 가장 효과적인 개선은 **프롬프트 보강**이며, 이는 코드 변경 없이 YAML 파일 수정만으로 가능합니다.
...
| 순위 | 조치 | 영향 | 난이도 |
|------|------|------|--------|
| 1 | **router_v1.yaml general 예시 보강** | 오분류 감소 | ⭐ |
| 2 | **Few-shot 예시 추가** | 분류 정확도 향상 | ⭐ |
| 3 | **st_gate "no_st" 시 LLM 스킵** | 비용/지연 감소 | ⭐⭐ |

#### 단기 개선 (Medium Effort)

| 순위 | 조치 | 영향 | 난이도 |
|------|------|------|--------|
| 4 | **route 기반 조건부 엣지** | 가시성/모듈성 향상 | ⭐⭐ |
| 5 | **route 메타데이터 → 검색 필터** | 검색 정확도 향상 | ⭐⭐ |
| 6 | **Judge 프롬프트 YAML 이동** | 유지보수성 향상 | ⭐⭐ |
...
| 7 | **노드별 모델 분리** | 비용 최적화 | ⭐⭐⭐ |
| 8 | **검색 병렬화** | 지연 시간 감소 | ⭐⭐⭐ |
| 9 | **임베딩 기반 라우팅** | 일관성/속도 향상 | ⭐⭐⭐ |
| 10 | **데이터 조회 카테고리 추가** | 기능 확장 | ⭐⭐⭐ |
...
**강력 권장.** 불필요한 LLM 호출 제거는 명확한 이점. 구현도 간단.
...
1. **즉시 적용**: router_v1.yaml 프롬프트 보강 (general 예시 + Few-shot)
2. **단기 적용**: st_gate 최적화, route 기반 검색 필터
3. **장기 검토**: 명시적 분기, 모델 분리, 임베딩 라우팅
