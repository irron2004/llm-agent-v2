...
| **TS** | 79 | 9p | 72% (알파벳 A.B.C.) | 3% (헤더) / **서브섹션 `X-N.` 패턴 풍부** | **중간** — 알파벳 TOC + `A-1.`/`B-1.` 서브섹션 패턴으로 섹션 경계 추출 가능 |
...
- 내부 페이지에서 `## A.` 같은 헤더는 3%만 존재하지만, **서브섹션 패턴 `X-N.`이 풍부**
  - 예: `A-1. Side door`, `A-2. FFU GUI parameter`, `B-1. Flange Adaptor`
- **추출 방법**: 각 페이지에서 `X-N.` 패턴의 첫 알파벳(X)을 추출 → TOC의 알파벳과 매칭
  - page에 `A-1.`, `A-2.`가 있으면 → `section_chapter = "A. FFU Pressure range error"`
  - page에 `B-1.`이 처음 등장하면 → `section_chapter = "B. AM Chamber Leak Trace"`
- 실측 결과 (5개 TS 문서): 서브섹션 패턴으로 페이지별 섹션 경계가 명확히 구분됨
- 알파벳 TOC가 없는 28%의 TS 문서는 기존 **인접 페이지 윈도우 확장** 유지
...
4. **Carry-forward 상속 (4순위)**
   - 1~3순위 실패 시 이전 페이지의 `section_chapter` 상속
